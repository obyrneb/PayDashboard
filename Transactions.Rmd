...
# Save combined plot
ggsave(plot = payplots, "Efficiency of Transaction Processing at the Pay Centre (May 2017 - Sep 2018).pdf", width = 8, height = 8)
...
pay_bklg.df <- pay.df %>%
  select(month, received, processed) %>%
  mutate(min = pmin(received, processed)) %>%
  gather("status", "transactions", -month, -min)

backlogplot <- ggplot(data=pay_bklg.df, aes(x=month, fill=status)) +
  geom_ribbon(aes(ymax=transactions, ymin=min)) +
  theme_TBS()
