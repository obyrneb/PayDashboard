...
```{r}
miniBacklog.df <- pay_eff.df %>%
  select(month, backlog) %>%
  filter(month != as.Date("2017-05-01"))

miniBacklog_c.df <- miniBacklog.df %>%
  filter(month %in% c(as.Date("2017-06-01"),as.Date("2018-10-01")))

miniBacklog <- ggplot(miniBacklog.df, aes(x=month, y=backlog, group=backlog)) +
  geom_path(aes(group=1)) +
  labs(title = "Backlog of Pay Centre Transactions") +
  ...
  scale_x_date(date_labels="%b %Y", expand=c(0,30),
               breaks=c(as.Date("2017-06-01"),as.Date("2018-01-01"),as.Date("2018-10-01"))) +
  geom_point(data=miniBacklog_c.df, aes(x=month, y=backlog, group=backlog)) +
  geom_text(data=miniBacklog_c.df, vjust = -0.5, size = 4,
            aes(x=month, y=backlog, label=round(backlog,0))) +
  geom_path(data=miniBacklog_c.df, colour="red", linetype=2, aes(group=1))+
  theme(
    strip.text.y = element_text(angle=0),
    strip.background = element_blank(),
    axis.title = element_blank()
